...
router.get("/",(req,res)=>{
	connection.query("select * from product",(err,records,fields)=>{
		if(err){
			console.error("Error while feteching data");
		}else{
			res.send(records);
		}
	})
...
router.post("/",(req,res)=>{
	...
	var sql = "INSERT INTO product VALUES (3, 'laptop', 'Good', 1000)";
...
router.put("/",(req,res)=>{
	...
	connection.query("update product set name = ?, price = ? where id= ?", [name,price,id], (err,result)=>{
		if(err){
